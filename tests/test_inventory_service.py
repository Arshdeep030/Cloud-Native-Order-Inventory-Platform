...
def test_inventory_creation_and_retrieval(inv_db):
    inv = create_inventory(inv_db, product_id=1, quantity=100)
    assert inv.id is not None
    assert inv.product_id == 1
    assert inv.quantity == 100
    assert inv.reserved_quantity == 0

    fetched = get_inventory(inv_db, product_id=1)
    assert fetched is not None
    assert fetched.quantity == 100
    assert fetched.reserved_quantity == 0
